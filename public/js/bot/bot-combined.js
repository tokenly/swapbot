(function() {
  ...
  swapbot.swapUtils = (function() {
    ...
    buildDesc.rate = function(swapConfig) {
      var formatCurrency, inAmount, normalizedInAmount, normalizedOutAmount, outAmount, _ref;
      outAmount = 1 * swapConfig.rate;
      inAmount = 1;
      formatCurrency = swapbot.formatters.formatCurrency;
      _ref = normalizeInAndOutQuantities(outAmount, inAmount), normalizedOutAmount = _ref[0], normalizedInAmount = _ref[1];
    ...
    buildDesc.fiat = function(swapConfig) {
      var cost, formatCurrency, formatFiatCurrency, normalizedInAmount, normalizedOutAmount, outAmount, _ref;
      formatCurrency = swapbot.formatters.formatCurrency;
      formatFiatCurrency = swapbot.formatters.formatArbitraryPrecisionFiatCurrency;
      outAmount = 1;
      cost = swapConfig.cost;
      _ref = normalizeInAndOutQuantities(outAmount, swapConfig.cost), normalizedOutAmount = _ref[0], normalizedInAmount = _ref[1];
    ...
    RecentOrActiveSwapComponent = React.createClass({
      ...
      render: function() {
        ...
        return React.createElement("li", {
          ...
          "className": "date"
        }, this.state.fromNow), React.createElement("span", null, (swap.isError || !swap.isComplete ? swap.message : swap.type === 'refund' ? "Refunded " + (swapbot.formatters.formatCurrency(swap.quantityOut)) + " " + swap.assetOut : "Sold " + (swapbot.formatters.formatCurrency(swap.quantityOut)) + " " + swap.assetOut + " for " + (swapbot.formatters.formatCurrency(swap.quantityIn)) + " " + swap.assetIn), (swap.isComplete ? React.createElement("a", {
  ...
  (function() {
    ...
    return SwapbotConfirmWallet = React.createClass({
      ...
      listenForKeyboardShortcuts: function() {
        $(document).on('keydown.confirmwallet', function(e) {
          if (e.keyCode === 89) {
            UserInputActions.confirmWallet();
          }
        });
      },
      stopListeningForKeyboardShortcuts: function() {
  ...
  Pockets = (function() {
    ...
    buildPromoLink = function() {
      ...
      return React.createElement('a', {
        ...
        React.createElement('img', {
          src: '/images/pockets/paywithpockets-blue.png',
          height: '32px',
          'width': '87px'
        })
      ]);
